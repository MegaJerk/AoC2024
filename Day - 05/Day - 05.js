...
//  page number X must be printed at some point before
//  page number Y.
//  
...
//  each update being printed. Because you are currently only printing
...
//  get if you add up the middle page number from those
...
//  you add up the middle page numbers after correctly ordering just
//  those updates?
/////////////////////////////////////////////////

/////////////////////////////////////////////////
////// Day 05 - INPUTS (TEST INPUTS)/////////////
/////////////////////////////////////////////////
var orderingRulesTest = [
  [47,53],
  [97,13],
  [97,61],
  [97,47],
...
////// Day 05 - VARIABLES ///////////////////////
/////////////////////////////////////////////////
var validUpdates, validatedUpdatesTest, sortedUpdates, sortedUpdatesTest;

/////////////////////////////////////////////////
////// Day 05 - FUNCTIONS ///////////////////////
/////////////////////////////////////////////////
function getOrderingRuleObject (orders){
  var ruleObject = {};

  orders.forEach(function(order){
    var firstPage = order[0];
    var secondPage = order[1];

    if (ruleObject[firstPage] === undefined) {
      ruleObject[firstPage] = [secondPage];
    } else {
      ruleObject[firstPage].push(secondPage);
    }
...
function validateUpdate(update, orderingRulesObject) {
  var validPages = [];

  update.forEach(function(page, pageIndex){

    var pageRules = (orderingRulesObject[page] ?? []).reduce(function(accumulator, currentValue){
      let updatePosition = update.indexOf(currentValue);
      if (updatePosition > -1) {
        accumulator.push(updatePosition);
      }
      return accumulator;
    }, []);
    ...
    while (validPage && i < pageRules.length) {
      if (pageRules[i] <= pageIndex) {
        validPage = false;
      }
  ...
  return (validPages.length === update.length);
}

function validateAllUpdates (updates, orderRules){
  ...
  updates.forEach(function(update, updateIndex){
    var validUpdate = validateUpdate(update, getOrderingRuleObject(orderRules));
    if (validUpdate) {
      validatedUpdates.middlePagesValue += update[((update.length - 1) / 2)];
      validatedUpdates.validUpdates.push(update);
    } else {
      validatedUpdates.invalidUpdates.push(update);
    }
...
function sortBadUpdates (updates, orderingRules) {

  var sortedUpdatesObject = {
    "sortedUpdates": [],
    "middlePagesValue": 0
  };

  var orderingRuleObject = getOrderingRuleObject(orderingRules);
  
  // Just write a damn compareFunction for the built in sorter
  // No need to write my own :)
  function compareFunction(page2, page1) {
    /*
      A negative value indicates that page2 should come before page1.
      A positive value indicates that page2 should come after page1.
      Zero or NaN indicates that a and b are considered equal.

      [97,13,75,29,47] -> [97, 75, 47, 29, 13]
    */

    var orderingRulesPage1 = orderingRuleObject[page1] ?? [];
    var orderingRulesPage2 = orderingRuleObject[page2] ?? [];

    if (orderingRulesPage2.indexOf(page1) > -1) {
      return -1; // page2 before page1
    }

    if (orderingRulesPage1.indexOf(page2) > -1) {
      return 1; // page1 before page2
    }
  ...
  updates.forEach(function(update){
    var sortedUpdate = update.toSorted(compareFunction);
    sortedUpdatesObject.sortedUpdates.push(sortedUpdate);
    sortedUpdatesObject.middlePagesValue += sortedUpdate[((sortedUpdate.length - 1) / 2)];
  });

  return sortedUpdatesObject;
}

/////////////////////////////////////////////////
////// Day 05 - Answer 01 ///////////////////////
/////////////////////////////////////////////////
validUpdates = validateAllUpdates(updates, orderingRules); // validUpdates.middlePages === 4569
validatedUpdatesTest = validateAllUpdates(updatesTest, orderingRulesTest); // validatedUpdatesTest.middlePages === 143

/////////////////////////////////////////////////
////// Day 05 - Answer 02 ///////////////////////
/////////////////////////////////////////////////
sortedUpdates = sortBadUpdates(validUpdates.invalidUpdates, orderingRules); // sortedUpdates.middlePages === 6456
sortedUpdatesTest = sortBadUpdates(validatedUpdatesTest.invalidUpdates, orderingRulesTest); // sortedUpdatesTest.middlePages === 123

console.log(`validUpdates.middlePagesValue: ${validUpdates.middlePagesValue}
sortedUpdates.middlePagesValue: ${sortedUpdates.middlePagesValue}`);